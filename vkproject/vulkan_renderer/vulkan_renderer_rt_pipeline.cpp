...
void VulkanRTRenderer::RT_Pipeline::Create() 
{
    ...
    pushConstant.stageFlags = vk::ShaderStageFlagBits::eRaygenKHR | vk::ShaderStageFlagBits::eClosestHitKHR;
    ...
    auto AlignUp = [](uint32_t x, size_t a) { return uint32_t((x + (uint32_t(a) - 1)) & ~uint32_t(a - 1)); };
    auto AlignUp64 = [](uint64_t x, size_t a) { return uint64_t((x + (uint64_t(a) - 1)) & ~uint64_t(a - 1)); };
...
void VulkanRTRenderer::RT_Pipeline::Submit(const glm::mat4 & P, const glm::mat4 & V, vk::Image & render_target)
{
    ...
    command_buffer.pushConstants<PushConstants>(
        pipeline_layout, vk::ShaderStageFlagBits::eRaygenKHR | vk::ShaderStageFlagBits::eClosestHitKHR, 0, pc);
